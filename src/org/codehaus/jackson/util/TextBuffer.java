 ...
 * <ul>
 * <li>TextBuffer uses segments character arrays, to avoid having to do additional array copies when array is not big
 * enough. This means that only reallocating that is necessary is done only once: if and when caller wants to access
 * contents in a linear array (char[], String).
 * </li>
 * <li>TextBuffer can also be initialized in "shared mode", in which it will just act as a wrapper to a single char
 * array managed by another object (like parser that owns it)
 * </li>
...
public final class TextBuffer {
     ...
     */
    /**
     * String that will be constructed when the whole contents are needed; will be temporarily stored in case asked for
     * again.
     */
     ...
     * next time a buffer is needed, buffers need to reallocated. Note: calling this method automatically also clears
 ...
 /* 28-Aug-2009, tatu: Actually, the current segment should
     ...
     */
    /**
     * Method called to make sure that buffer is not using shared input buffer; if it is, it will copy such contents to
     * private buffer.
     */
     ...
     */
    /**
     * Note: calling this method may not be as efficient as calling {@link #contentsAsString}, since it's not guaranteed
     * that resulting String is cached.
     */
    @Override
