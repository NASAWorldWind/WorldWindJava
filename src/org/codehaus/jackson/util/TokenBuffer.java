...
/**
 * Utility class used for efficient storage of {@link JsonToken} sequences, needed for temporary buffering. Space
 * efficient for different sequence lengths (especially so for smaller ones; but not significantly less efficient for
 * larger), highly efficient for linear iteration and appending. Implemented as segmented/chunked linked list of tokens;
 * only modifications are via appends.
 *
 * @since 1.5
 */
 ...
 * checking
    ...
    ***********************************************
     */
    /**
     * Object codec to use for stream-based object conversion through parser/generator interfaces. If null, such methods
     * can not be used.
     */
    ...
    ***********************************************
    ...
    /**
     * Method used to create a {@link JsonParser} that can read contents stored in this buffer. Will use default
     * <code>_objectCodec</code> for object conversions.
     * <p>
     * Note: instances are not synchronized, that is, they are not thread-safe if there are concurrent appends to the
     * underlying buffer.
     *
    ...
    /**
     * Method used to create a {@link JsonParser} that can read contents stored in this buffer.
     * <p>
     * Note: instances are not synchronized, that is, they are not thread-safe if there are concurrent appends to the
     * underlying buffer.
     *
    ...
    *****************************************************************
     */
    /**
     * Helper method that will write all contents of this buffer using given {@link JsonGenerator}.
     * <p>
     * Note: this method would be enough to implement <code>JsonSerializer</code> for <code>TokenBuffer</code> type; but
     * we can not have upwards references (from core to mapper package); and as such we also can not take second
     * argument.
     *
    ...
    *****************************************************************
     ...
     * requirement). Current implementation uses fixed length array; could alternatively use 16 distinct fields and
     * switch statement (slightly more efficient storage, slightly slower access)
     */
