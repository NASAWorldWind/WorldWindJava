...
/**
 * Base class for all JSON nodes, which form the basis of JSON Tree Model that Jackson implements. One way to think of
 * these nodes is to considere them similar to DOM nodes in XML DOM trees.
 * <p>
 * As a general design rule, most accessors ("getters") are included in this base class, to allow for traversing
 * structure without type casts. Most mutators, however, need to be accessed through specific sub-classes. This seems
 * sensible because proper type information is generally available when building or modifying trees, but less often when
 * reading a tree (newly built from parsed Json content).
 */
public abstract class JsonNode
    ...
    /**
     * Method that returns true for "virtual" nodes which represent missing entries constructed by path accessor methods
     ...
     * true only for instances of org.codehaus.jackson.node.POJONode.
     *
    ...
    /**
     * @return True if this node represents an integral numeric Json value that withs in Java int value space
     */
    ...
    /**
     * @return True if this node represents an integral numeric Json value that fits in Java long value space (but not
     * int value space, i.e. {@link #isInt} returns false)
     */
    ...
    /**
     * If this node is a numeric type (as per {@link #isNumber}), returns native type that node uses to store the
     * numeric value.
     *
     * @return Undocumented.
     */
     ...
     * for Text Nodes (ones for which {@link #getTextValue} returns non-null value), to read decoded base64 data.For
     * other types of nodes, returns null.
     *
     ...
     * this node is numeric ({@link #isNumber} returns true); otherwise returns null
     *
    ...
    /**
     * Method for accessing value of the specified element of an array node.For other nodes, null is always returned.<p>
     ...
     * index is less than 0, or equal-or-greater than <code>node.size()</code>, null is returned; no exception is thrown
     * for any index.
     *
     ...
     * {@link #isValueNode} returns true), otherwise null.
     * <p>
     * Note: to serialize nodes of any type, you should call {@link #toString} instead.
     *
     * @return Undocumented.
     */
     ...
     */
    /**
     * Method that returns number of child nodes this node contains: for Array nodes, number of child elements, for
     * Object nodes, number of fields, and for all other nodes 0.
     *
     * @return For non-container nodes returns 0; for arrays number of contained elements, and for objects number of
     * fields.
     */
    public int size() {
        return 0;
    ...
    /**
     * Same as calling {@link #getElements}; implemented so that convenience "for-each" loop can be used for looping
     * over elements of Json Array constructs.
     */
    @Override
     ...
     * @deprecated Use methods that are part of {@link JsonGenerator} or org.codehaus.jackson.map.ObjectMapper instead.
     */
