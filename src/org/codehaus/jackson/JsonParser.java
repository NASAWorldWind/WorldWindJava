...
/**
 * Base class that defines public API for reading JSON content. Instances are created using factory methods of a
...
public abstract class JsonParser
    ...
    public enum Feature {
         ...
         * the parser. If disabled, calling application has to separately close the underlying {@link InputStream} and
         * {@link Reader} instances used to create the parser. If enabled, parser will handle closing, as long as parser
         ...
         * <p>
         * Since JSON specification does not mention comments as legal construct, this is a non-standard feature;
         * however, in the wild this is extensively used. As such, feature is
         * <b>disabled by default</b> for parsers and must be explicitly enabled (via factory or parser instance).
         * <p>
         * This feature can be changed for parser instances.
         */
        , ALLOW_COMMENTS(false) /**
         * Feature that determines whether parser will allow use of unquoted field names (which is allowed by
         * Javascript, but not by JSON specification).
         * <p>
         * Since JSON specification requires use of double quotes for field names, this is a non-standard feature, and
         * as such disabled by default.
         * <p>
         * This feature can be changed for parser instances.
         *
         * @since 1.2
         ...
         * Feature that determines whether parser will allow use of single quotes (apostrophe, character '\'') for
         * quoting Strings (names and String values). If so, this is in addition to other acceptabl markers. but not by
         * JSON specification).
         * <p>
         * Since JSON specification requires use of double quotes for field names, this is a non-standard feature, and
         * as such disabled by default.
         ...
         * Feature that determines whether parser will allow JSON Strings to contain unquoted control characters (ASCII
         * characters with value less than 32, including tab and line feed characters) or not. If feature is set false,
         * an exception is thrown if such a character is encountered.
         * <p>
         * Since JSON specification requires quoting for all control characters, this is a non-standard feature, and as
         ...
         * Feature that determines whether JSON object field names are to be canonicalized using {@link String#intern}
         ...
         * <p>
         * Note: this setting only has effect if {@link #CANONICALIZE_FIELD_NAMES} is true -- otherwise no
         * canonicalization of any sort is done.
         *
         * @since 1.3
         */
        , INTERN_FIELD_NAMES(true) /**
         * Feature that determines whether JSON object field names are to be canonicalized (details of how
         * canonicalization is done then further specified by {@link #INTERN_FIELD_NAMES}).
         *
         * @since 1.5
         */
        , CANONICALIZE_FIELD_NAMES(true) // 14-Sep-2009, Tatu: This would be [JACKSON-142] implementation:
        /*
     ...
     */
    /**
     * Bit flag composed of bits that indicate which {@link org.codehaus.jackson.JsonParser.Feature}s are enabled.
     */
     ...
     */
    /**
     * Last token retrieved via {@link #nextToken}, if any. Null before the first call to <code>nextToken()</code>, as
     * well as if token has been explicitly cleared (by call to {@link #clearCurrentToken})
     */
     ...
     * source if parser either <b>owns</b> the input source, or feature {@link Feature#AUTO_CLOSE_SOURCE} is enabled.
     * Whether parser owns the input source depends on factory method that was used to construct instance (so check
     * {@link org.codehaus.jackson.JsonFactory} for details, but the general idea is that if caller passes in closable
     * resource (such as {@link InputStream} or {@link Reader}) parser does NOT own the source; but if it passes a
     * reference (such as {@link java.io.File} or {@link java.net.URL} and creates stream or reader it does own them.
     */
    @Override
     ...
     *
     * @return Type of the token this parser currently points to, if any: null before any tokens have been read, and
     * after end-of-input has been encountered, as well as if the current token has been explicitly cleared.
     */
     ...
     * to check for <code>parser.getCurrentToken() != null</code>.
     *
     * @return True if the parser just returned a valid token via {@link #nextToken}; false otherwise (parser was just
     * constructed, encountered end-of-input and returned null from {@link #nextToken}, or the token has been consumed)
     */
    ...
    /**
     * Method called to "consume" the current token by effectively removing it so that {@link #hasCurrentToken} returns
     * false, and {@link #getCurrentToken} null). Cleared token value can still be accessed by calling
     * {@link #getLastClearedToken} (if absolutely needed), but usually isn't.
     * <p>
    ...
    /**
     * Method that can be called to get the name associated with the current token: for {@link JsonToken#FIELD_NAME}s it
     * will be the same as what {@link #getText} returns; for field values it will be preceding field name; and for
     * others (array values, root-level values) null.
     *
     * @return Undocumented.
     * @throws java.io.IOException Undocumented.
     * @throws org.codehaus.jackson.JsonParseException Undocumented.
    ...
    /**
     * Method that return the <b>starting</b> location of the current token; that is, position of the first character
     ...
     * {@link #nextToken}, or after encountering end-of-input), returns null.Method can be called for any token type.
     *
     * @return Undocumented.
     * @throws java.io.IOException Undocumented.
     * @throws org.codehaus.jackson.JsonParseException Undocumented.
    ...
    /**
     * Accessor used with {@link #getTextCharacters}, to know length of String stored in returned buffer.
     *
     * @return Number of characters within buffer returned by {@link #getTextCharacters} that are part of textual
    ...
    /**
     * Accessor used with {@link #getTextCharacters}, to know offset of the first text content character within buffer.
     *
     * @return Offset of the first character within buffer returned by {@link #getTextCharacters} that is part of
    ...
    /**
     * If current token is of type {@link JsonToken#VALUE_NUMBER_INT} or {@link JsonToken#VALUE_NUMBER_FLOAT}, returns
    ...
    /**
     * Convenience alternative to {@link #getBinaryValue(Base64Variant)} that defaults to using
     ...
     * org.codehaus.jackson.map.MappingJsonFactory but not for {@link JsonFactory} (unless its <code>setCodec</code>
     ...
     * is true for parsers constructed by org.codehaus.jackson.map.MappingJsonFactory but not for {@link JsonFactory}
     * (unless its <code>setCodec</code> method has been explicitly called).
     * <p>
     * This method may advance the event stream, for structured types the current token will be the closing end marker
     * (END_ARRAY, END_OBJECT) of the bound structure. For non-structured Json types (and for
     * {@link JsonToken#VALUE_EMBEDDED_OBJECT}) stream is not advanced.
