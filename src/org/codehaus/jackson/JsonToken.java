...
public enum JsonToken {
    ...
    /**
     * NOT_AVAILABLE can be returned if {@link JsonParser} implementation can not currently return the requested token
     * (usually next one), or even if any will be available, but that may be able to determine this in future. This is
     * the case with non-blocking parsers -- they can not block to wait for more data to parse and must return
     * something.
     *
     * @since 0.9.7
     */
    ...
    END_ARRAY("]"),
    /**
     * FIELD_NAME is returned when a String token is encountered as a field name (same lexical value, different
     * function)
     */
    FIELD_NAME(null),
    /**
     * Placeholder token returned when the input source has a concept of embedded Object that are not accessible as
     * usual structure (of starting with {@link #START_OBJECT}, having values, ending with {@link #END_OBJECT}), but as
     * "raw" objects.
     * <p>
     * Note: this token is never returned by regular JSON readers, but only by readers that expose other kinds of source
     * (like {@link JsonNode}-based JSON trees, Maps, Lists and such).
     *
     * @since 1.1
     */
    VALUE_EMBEDDED_OBJECT(null),
    /**
     * VALUE_STRING is returned when a String token is encountered in value context (array element, field value, or
     * root-level stand-alone value)
     */
    VALUE_STRING(null),
    /**
    ...
    /**
     * VALUE_NUMBER_INT is returned when a numeric token other that is not an integer is encountered: that is, a number
     * that does have floating point or exponent marker in it, in addition to one or more digits.
     */
    ...
    /**
     * @param Textual representation for this token, if there is a single static representation; null otherwise
     */
