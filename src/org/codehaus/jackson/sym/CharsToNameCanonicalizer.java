 ...
 * <p>
 * For optimal performance, usage pattern should be one where matches should be very common (esp. after "warm-up"), and
 * as with most hash-based maps/sets, that hash codes are uniformly distributed. Also, collisions are slightly more
 * expensive than with HashMap or HashSet, since hash codes are not used in resolving collisions; that is, equals()
 * comparison is done with all symbols in same bucket index.<br>
 * Finally, rehashing is also more expensive, as hash codes are not stored; rehashing requires all entries' hash codes
 * to be recalculated. Reason for not storing hash codes is reduced memory usage, hoping for better memory locality.
 * <p>
 ...
 * concurrently with child instances can only be done if access to master instance is read-only (ie. no modifications
 * done).
 */
public final class CharsToNameCanonicalizer {

    /**
     * Default initial table size. Shouldn't be miniscule (as there's cost to both array realloc and rehashing), but
     * let's keep it reasonably small nonetheless. For systems that properly reuse factories it doesn't matter either
     * way; but when recreating factories often, initial overhead may dominate.
     */
     ...
     * linkage is defined, and child instance is released (call to <code>release</code>), parent's shared tables may be
     * updated from the child instance.
     */
    ...
    /**
     * Mask used to get index from hash values; equal to <code>_buckets.length - 1</code>, when _buckets.length is a
     * power of two.
     */
     ...
     */
    /**
     * Flag that indicates if any changes have been made to the data; used to both determine if bucket array needs to be
     * copied when (first) change is made, and potentially if updated bucket list is to be resync'ed back to master
     * instance.
     */
     ...
     *
     * @param initialSize Minimum initial size for bucket array; internally will always use a power of two equal to or
     ...
     * has to remain power of two, arrays will then always be doubled. Main work is really redistributing old entries
