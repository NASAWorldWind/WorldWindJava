...
/**
 * Intermediate base class used by all Jackson {@link JsonParser} implementations. Contains most common things that are
 * independent of actual underlying input source
 *
...
public abstract class JsonParserBase
     ...
     */
    /**
     * Number of characters that were contained in previous blocks (blocks that were already processed prior to the
     * current buffer).
     */
    ...
    /**
     * Total number of characters read before start of current token. For big (gigabyte-sized) sizes are possible, needs
     * to be long, unlike pointers and sizes related to in-memory buffers.
     */
    ...
    /**
     * Secondary token related to the current token: used when the current token is <code>FIELD_NAME</code> but the
     ...
     */
    /**
     * Buffer that contains contents of String values, including field names if necessary (name split across boundary,
     * contains escape sequence, or access needed to char array)
     */
    protected final TextBuffer _textBuffer;

    /**
     * Temporary buffer that is needed if field name is accessed using {@link #getTextCharacters} method (instead of
     * String returning alternatives)
     */
    protected char[] _nameCopyBuffer = null;

    /**
     * Flag set to indicate whether the field name is available from the name copy buffer or not (in addition to its
     * String representation being available via read context)
     */
     ...
     * {@link #getBinaryValue} will not need to decode data more than once.
     */
     ...
     * {@link #nextToken}, or after encountering end-of-input), returns null. Method can be called for any event.
     */
