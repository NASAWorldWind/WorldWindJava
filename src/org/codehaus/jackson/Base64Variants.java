...
public final class Base64Variants {
     ...
     * <p>
     * Note that although this can be thought of as the standard variant, it is <b>not</b> the default for Jackson:
     * no-linefeeds alternative is because of JSON requirement of escaping all linefeeds.
     */
    ...
    /**
     * Slightly non-standard modification of {@link #MIME} which does not use linefeeds (max line length set to
     * infinite). Useful when linefeeds wouldn't work well (possibly in attributes), or for minor space savings (save 1
     * linefeed per 76 data chars, ie. ~1.4% savings).
     */
    ...
    /**
     * This variant is the one that predates {@link #MIME}: it is otherwise identical, except that it mandates shorter
     * line length.
     */
     ...
     * request). It differs from the base {@link #MIME} variant in multiple ways. First, no padding is used: this also
     * means that it generally can not be written in multiple separate but adjacent chunks (which would not be the usual
     * use case in any case). Also, no linefeeds are used (max line length set to infinite). And finally, two characters
    ...
    /**
     * Method used to get the default variant ("MIME_NO_LINEFEEDS") for cases where caller does not explicitly specify
     * the variant.We will prefer no-linefeed version because linefeeds in JSON values must be escaped, making
     * linefeed-containing variants sub-optimal.
     *
     * @return Undocumented.
     */
