...
public class NITFSImageSegment extends NITFSSegment {
                ...
                for (int row = 0; row < this.numOfImageRows; row++) {
                    ...
                    for (int col = 0; col < this.numOfImageCodesPerRow; col++) {
                        if (hasTransparentPixels) {
                            if (4095 == codes[col]) {   // this is a transparent kernel
                                continue;
                            }
                        }
                        ...
                        for (int h = 0; h < 4; h++) {
                            for (int w = 0; w < 4; w++) {
                                ...
                                if (hasTransparentPixels) {
                                    if (this.transparentOutputPixelCode == colorCode) {   // this is a transparent pixel
                                        continue;
                                    }
                                }
    ...
    private void parseImageData(java.nio.ByteBuffer buffer) throws NITFSRuntimeException {
        ...
        if (0 < componentLocationTable.getImageDisplayParametersSubheaderLength()) {   // parse [ nitf-rpf image display parameter sub-header ]
