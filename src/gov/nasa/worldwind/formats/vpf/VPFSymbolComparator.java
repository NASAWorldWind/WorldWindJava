 ...
 * features being displayed. GeoSym uses four "nested" methods to define the order in which symbols should be displayed.
 * <p>
 ...
 * higher priority should be displayed after (on top of) one with a lower priority. For DNC, the values contained in the
 * display priority field are defined based on the S52 lookup tables. Each DNC feature/attribute was mapped to a
 ...
 * <p>
 * 6.5.3.3 Features with Text Labels As a general rule, the display priority specified for a feature's text label(s) is
 * the highest value possible, regardless of the display priority assigned to the base feature. In DNC, all text labels
 * will have a display priority of 8 (per IHO S52). In all other products, text labels have a display priority of 9.
 * Therefore, the text labels will by default be displayed on top of all other symbols since the display priority value
 * is the first criterion controlling the order of symbology display on the screen. However, it is possible that a
 ...
 * pattern fill. Except for the special case where an area feature's symbology requires both a solid and pattern fill(s)
 * (see section 6.5.3.2), all symbols for an area feature will be specified in the same row of the *sym.txt file. In
 * other words, any or all of the symbol columns may be populated. In these cases, the symbols are to be displayed
 * according to the same "painter's algorithm" that applies at the feature level: the area symbol (from areasym) should
 * be drawn first, followed by the linear symbol (from linesym) followed by the point symbol (from pointsym).
 *
 * @author dcollins
