...
public class ShapefileMultiPatch extends Mesh3D {
    ...
    protected PatchGeometry generateTriangleStrip(Terrain terrain, Position[] locations, Vec4 referencePoint) {
        ...
        for (int i = 0; i < locations.length - 2; i++) {
            for (int j = 0; j < 3; j++) {
                trianglePoints[j] = this.computePoint(terrain, locations[i + j]).subtract3(referencePoint);
            }
...
//            System.out.println("tp0=" + trianglePoints[0]);
//            System.out.println("tp1=" + trianglePoints[1]);
//            System.out.println("tp2=" + trianglePoints[2]);
            trianglePoints = this.addTriangle(trianglePoints, vertices);
            addTriangleNormals(trianglePoints, normals);
        }
        return new PatchGeometry(vertices, normals);
    ...
    /**
     * Invalidates computed values. Called when this shape's contents change.
     */
    @Override
...
//            Sector sector = Sector.boundingSector(globe, globe.computePositionFromPoint(ext.getCenter()), ext.getDiameter() / 2);
//            double[] minMaxElev = globe.getMinAndMaxElevations(sector);
//            Position modelPos = this.getModelPosition();
//            surfacePos = new Position(modelPos.latitude, modelPos.longitude, minMaxElev[1]);
//
//        } else {
//            surfacePos = this.getModelPosition();
//        }
    ...
    protected boolean doMakeOrderedRenderable(DrawContext dc) {
        if (this.patchGeometries == null || this.mustRegenerateGeometry(dc)) {
    ...
    protected static ArrayList<Record> assembleRecords(Shapefile shapefile) {
        ArrayList<Record> records = new ArrayList<>();

        while (shapefile.hasNext()) {
            ShapefileRecord shapefileRecord = shapefile.nextRecord();

            if (mustAssembleRecord(shapefileRecord)) {
    ...
    public static ArrayList<ShapefileMultiPatch> createMeshes(Shapefile shapefile, ShapeAttributes normalAttrs, ShapeAttributes highlightAttrs,
            ShapefileRenderable.AttributeDelegate attributeDelegate) {
        ArrayList<Record> records = assembleRecords(shapefile);
        ArrayList<ShapefileMultiPatch> shapes = new ArrayList<>();
        records.forEach((record) -> {
            ShapefileMultiPatch shape = new ShapefileMultiPatch(record, normalAttrs, highlightAttrs, attributeDelegate);
