...
/**
 * Represents a Shapefile record with a multi patch shape type. Multi-patch
 * shapes represent a set of x,y coordinate pairs.
 * <p>
 * Multi-patchs may have optional z-coordinates or m-coordinates that accompany
 * each x,y coordinate pair. If a Multi-patch has z-coordinates, then
 * <code>{@link #getZValues()}</code> returns a non-<code>null</code> array of
 * values. If a Multi-patch has m-coordinates, then
 * <code>{@link #getMValues()}</code> returns a non-<code>null</code> array of
 * values.
 *
 */
public class ShapefileRecordMultiPatch extends ShapefileRecord {
    ...
    /**
     * Returns an iterator over all the points X and Y coordinates for a
     * specified part of this record. Part numbers start at zero.
     *
     ...
     *
     * @return the shape's Z range. The range minimum is at index 0, the maximum
     * at index 1.
     */
     ...
     *
     * @return the shape's measure range, or null if no measures are in the
     * record. The range minimum is at index 0, the maximum at index 1.
     */
     ...
     *
     * @return the shape's measure values, or null if no measures are in the
     * record.
     */
