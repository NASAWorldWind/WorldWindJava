 ...
 * coordinates are interpreted as follows: <ul>
 * <li>Unspecified - coordinates are not changed.</li> <li>Geographic -
 ...
 * coordinate.</li> <li>Universal Transverse Mercator (UTM) - UTM coordinates
 * are converted to geographic coordinates during parsing.</li> <li>Unsupported
 ...
 * <li>{@link gov.nasa.worldwind.avlist.AVKey#PROJECTION_ZONE} - the UTM zone
 * (if coordinate system projection is UTM); an integer in the range 1-60.</li>
 * <li>{@link gov.nasa.worldwind.avlist.AVKey#PROJECTION_HEMISPHERE} - the UTM
...
public class Shapefile extends AVListImpl implements Closeable, Exportable {
     ...
     * <li>{@link File}</li> <li>{@link String} containing a valid URL
     * description or a file or resource name available on the classpath.</li>
     * </ul>
     * <p>
     * The source Shapefile may be accompanied by an optional index file,
     * attribute file, and projection file. To be recognized by this Shapefile,
     ...
     * otherwise one of the following symbolic constants: <ul>
     * <li>{@link #SHAPE_NULL}</li> <li>{@link #SHAPE_POINT}</li>
     * <li>{@link #SHAPE_MULTI_POINT}</li>
     * <li>{@link #SHAPE_POLYLINE}</li> <li>{@link #SHAPE_POLYGON}</li>
     * <li>{@link #SHAPE_POINT_M}</li> <li>{@link
     * #SHAPE_MULTI_POINT_M}</li> <li>{@link #SHAPE_POLYLINE_M}</li>
     * <li>{@link #SHAPE_POLYGON_M}</li> <li>{@link
     * #SHAPE_POINT_Z}</li> <li>{@link #SHAPE_MULTI_POINT_Z}</li>
     * <li>{@link #SHAPE_POLYLINE_Z}</li> <li>{@link
    ...
    /**
     * Returns <code>true</code> if the Shapefile has a more records, and
     * <code>false</code> if all records have been read.
     *
     * @return <code>true</code> if the Shapefile has a more records;
     ...
     * {@link #SHAPE_MULTI_POINT_Z}.</li>
     * <li>{@link gov.nasa.worldwind.formats.shapefile.ShapefileRecordPolyline}
     * if type is {@link #SHAPE_POLYLINE},
     * {@link #SHAPE_POLYLINE_M} or {@link #SHAPE_POLYLINE_Z}.</li>
     * <li>{@link gov.nasa.worldwind.formats.shapefile.ShapefileRecordPolygon}
     * if type is {@link #SHAPE_POLYGON}, {@link #SHAPE_POLYGON_M} or
     * {@link #SHAPE_POLYGON_Z}.</li> </ul>
     * <p>
     ...
     * are still available. The following methods are safe to call: <ul>
     ...
     *
     * @return <code>true</code> if the shapefile's points should be normalized;
     * <code>false</code> otherwise.
     */
     ...
     *
     * @param normalizePoints <code>true</code> if the shapefile's points should
     * be normalized; <code>false</code> otherwise.
     */
     ...
     * <code>null</code> if this Shapefile has no accompanying index file, if
     * the index file is empty, or if the JVM cannot allocate enough memory to
     * hold the index.
     *
     * @return the Shapefile's record offset index, or <code>null</code> if the
     ...
     * system parameters. This returns <code>null</code> if this Shapefile has
     ...
     * <code>null</code> if this Shapefile has no accompanying projection file
     ...
     * <code>SHAPE_POINT</code>, this returns a
     * <code>ShapefileRecordPoint</code>, and if the record's shape type is
     * <code>SHAPE_NULL</code>, this returns <code>ShapefileRecordNull</code>.
     * <p>
     * This returns <code>null</code> if the record's shape type is not one of
     * the following types: <code>SHAPE_POINT</code>,
     * <code>SHAPE_POINT_M</code>, <code>SHAPE_POINT_Z</code>,
     * <code>SHAPE_MULTI_POINT</code>, <code>SHAPE_MULTI_POINT_M</code>,
     * <code>SHAPE_MULTI_POINT_Z</code>, <code>SHAPE_NULL</code>,
     ...
     * <code>null</code> if the record's shape type is not one of the recognized
     * types.
     */
...
//                return SHAPE_MULTI_PATCH;
     ...
     * {@link ShapefileRecord#isNormalizePoints()} returns <code>true</code>.
     *
     * @param record the record associated with the point coordinates, may be
     * null.
     * @param buffer the buffer to read point coordinates from.
     ...
     * normalized. If <code>isNormalized</code> is <code>true</code>, this
