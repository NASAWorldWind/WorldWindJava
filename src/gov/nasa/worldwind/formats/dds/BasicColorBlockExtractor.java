...
public class BasicColorBlockExtractor implements ColorBlockExtractor {
    ...
    /**
     * Creates a <code>BasicColorBlockExtrator</code> which will draw its data from the <code>BufferedImage</code>. The
     * <code>BufferedImage</code> may be of any type, so long as a call to <code>image.getRGB()</code> will succeed.
     *
     ...
     * is not, this will either truncate the block to fit the image, or copy nearby pixels to fill the block. If the
     * <code>attributes</code> specify that color components should be premultiplied by alpha, this extactor will
     * perform the premultiplication operation on the incoming colors.
     * <p>
     * Access to this method must be synchronized by the caller. This method is frequenty invoked by the DXT compressor,
     * so in order to reduce garbage each instance of this class has unsynchronized properties that are reused during
     * each call.
     *
