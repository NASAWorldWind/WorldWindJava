 ...
 * elements until its size is equal to its capacity.
 * <p>
 * BoundedHashMap can operate in two ordering modes: insertion order and access order. The mode specified which entries
 * are automatically removed when the map is over capacity. In insertion order mode the map removes the eldest entry
 * (the first entry added). In access order mode, the map automatically removes the least recently used entry.
 *
 * @author dcollins
