 ...
 * <h2>Limits on the number of WebViews that can be created</h2> WindowsWebView creates a hidden native window. Creating
 * the native window can fail if the process runs out of Windows user object handles. Other GUI elements in an
 * application also consume these handles, so it is difficult to put a firm limit on how many WebViews can be created.
 * An application that creates only WebViews and no other windows can create about 1500 WebViews before running out of
 * handles. See <a href="http://msdn.microsoft.com/en-us/library/ms725486%28v=vs.85%29.aspx">MSDN</a> for more
 * information on User Objects and operating system limits.
 *
 * @author pabercrombie
...
public class WindowsWebView extends AbstractWebView {
     ...
     * @throws WWRuntimeException if creating the native web browser window fails for any reason. For example, because
     * the process has run out of User Object handles (see documentation <a href="#limits">above</a>).
     */
