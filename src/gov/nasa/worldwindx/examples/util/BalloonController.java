...
public class BalloonController extends MouseAdapter implements SelectListener {
     ...
     * take action on the event (by opening a KML document, etc). If the event is not a link activation trigger, but the
     * URL is a URL that the balloon controller would normally handle, the event is consumed to prevent the balloon
     * itself from trying to handle the event, but no further action is taken.
     ...
     * @param event The select event that activated the link. This event will be consumed if a KML feature is found that
     * matches the link fragment. However, the controller only moves to the feature or opens a balloon if the event is a
     * link activation event, or null. Other events are consumed to prevent the balloon from handling events for a link
     * that the controller wants handle. This parameter may be null.
     *
     ...
     * @param featureRef A reference to a feature in the remote file to animate the globe to once the file is available.
     *
    ...
    protected class DocumentRetrievalTask extends TimerTask {
         ...
         * (for example, "myFeature;flyto"). The action will be carried out when the document becomes available.
