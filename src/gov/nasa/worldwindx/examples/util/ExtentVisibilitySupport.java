 ...
 * of {@link gov.nasa.worldwind.geom.Extent} instances and
 * {@link gov.nasa.worldwindx.examples.util.ExtentVisibilitySupport.ScreenExtent} instances by invoking
 * {@link #setExtents(Iterable)} and {@link #setScreenExtents(Iterable)}, respectively. These Iterables defines
 * ExtentVisibilitySupport's scene elements. ExtentVisibilitySupport keeps a direct reference to these Iterables; it
 * does not attempt to copy or modify them in any way. Any null elements in these Iterables are ignored. The static
 * convenience method {@link #extentsFromExtentHolders(Iterable, gov.nasa.worldwind.globes.Globe, double)} makes it easy
 * to for callers to convert an Iterable of Extents references to an Iterable of {@link
...
public class ExtentVisibilitySupport {
     ...
     * Iterable, and any null Extents returned by the ExtentHolders. This returns null if the Iterable is null or empty.
     *
     ...
     * This iterable can be null, indicating that this ExtentVisibilitySupport has no model coordinate scene.
     */
     ...
     * scene elements. This iterable can be null, indicating that this ExtentVisibilitySupport has no screen coordinate
     * scene.
     */
     ...
     * are null, if if the eye point and reference center point are coincident, if the up vector and the line of sight
     * are parallel, or if any of the view projection parameters are out-of-range.
     */
